...
class InputResolutionTests(StageBatchSystemTestCase):
    def test_pipeline_dependency_resolution_is_contract_driven(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            resolved = resolve_stage_inputs_for_train_eval_pipeline(
                spec, plan, stage_name="eval"
            )

            self.assertEqual(len(resolved.selected_runs), 1)
            self.assertEqual(resolved.blocked_run_ids, ())
            binding = resolved.input_bindings_by_run[resolved.selected_runs[0].run_id][
                0
            ]
    ...
    def test_checkpoint_source_requires_explicit_input_name_when_ambiguous(
        self,
    ) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload["stages"]["eval"]["inputs"] = {
                "checkpoint": {
                    "source": {
                        "kind": "external_path",
                        "path": str(root / "missing_checkpoint.pt"),
                    },
                    "expects": "path",
                    "required": False,
                    "inject": {
                ...
                "adapter": {
                    "source": {
                        "kind": "external_path",
                        "path": str(root / "missing_adapter.pt"),
                    },
                    "expects": "path",
                    "required": False,
