...
class ExecutorTests(StageBatchSystemTestCase):
    def test_executor_runs_before_steps_with_stage_environment(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload["stages"]["train"]["before"] = [
                {"name": "prepare-marker", "run": "printf '%s' \"$DEMO_ENV\" > before_marker.txt"},
            ]
    ...
    def test_executor_runtime_contract_failure_blocks_user_script(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            probe = json.loads((train_run_dir / "attempts" / "0001" / "runtime_probe.json").read_text())
            probe_states = {item["runtime_role"]: item["state"] for item in probe["probes"]}
    ...
    def test_executor_preflight_missing_input_fails_before_user_script(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotEqual(execute_stage_task(Path(eval_batch.submission_root), 1, 0), 0)
    ...
    def test_submit_preflight_fails_on_expected_input_digest_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            materialization = json.loads((Path(eval_batch.submission_root) / "materialization_status.json").read_text())
