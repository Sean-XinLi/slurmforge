...
def launcher_plan_from_dict(payload: dict[str, Any]) -> LauncherPlan:
    ...
    return LauncherPlan(
        ...
        nproc_per_node=None if payload["nproc_per_node"] in (None, "") else int(payload["nproc_per_node"]),
        rendezvous=None
        ...
        master_port=None if payload["master_port"] in (None, "") else int(payload["master_port"]),
    )
