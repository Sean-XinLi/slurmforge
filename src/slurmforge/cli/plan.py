...
def handle_plan(args: argparse.Namespace) -> None:
    ...
    if command == "train":
        ...
        batch = build_eval_stage_batch(
            spec,
            from_train_batch=source.value if source is not None and source.kind == "from_train_batch" else None,
            from_run=source.value if source is not None and source.kind == "from_run" else None,
            checkpoint=source.value if source is not None and source.kind == "checkpoint" else None,
...
def add_subparser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    parser = subparsers.add_parser("plan", help="Compile plan and sbatch files without submitting")
    plan_subparsers = parser.add_subparsers(dest="plan_command")
    plan_subparsers.required = True

    train_parser = plan_subparsers.add_parser("train", help="Compile a train stage batch")
