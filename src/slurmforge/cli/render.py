...
def print_stage_batch_execution_result(result: StageBatchExecutionResult, *, noun: str = "stage batch") -> None:
...
def print_sourced_stage_batch_execution_result(result: SourcedStageBatchExecutionResult, *, noun: str) -> None:
...
def print_train_eval_pipeline_execution_result(result: TrainEvalPipelineExecutionResult) -> None:
    if result.mode == "preview":
        return
    if result.submitted:
        print(f"[OK] submitted train/eval pipeline controller: {result.controller_job_id}")
