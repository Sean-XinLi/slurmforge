...
def find_stage_instance(
    batch_root: Path, group_index: int, task_index: int
) -> StageInstancePlan:
    batch = load_execution_stage_batch_plan(batch_root)
    group = next(
        (item for item in batch.group_plans if item.group_index == group_index), None
    )
    if group is None:
        raise ValueError(f"No group_index={group_index} in {batch_root}")
    if task_index < 0 or task_index >= len(group.stage_instance_ids):
        raise ValueError(
            f"task_index={task_index} outside group array size {len(group.stage_instance_ids)}"
        )
    target = group.stage_instance_ids[task_index]
    return next(
        item for item in batch.stage_instances if item.stage_instance_id == target
    )
