    ...
    *,
    ...
    if input_spec is None:
        raise ConfigContractError(
            f"`stages.{consumer_stage}.inputs.{input_name}` is required"
        )
    if input_spec.expects not in {"path", "manifest"}:
        raise ConfigContractError(
            f"`{input_name}` expects {input_spec.expects}; explicit path inputs require path or manifest"
        )
    source_path = path.expanduser()
    resolved = (
        source_path.resolve()
        if source_path.is_absolute()
        else (spec.project_root / source_path).resolve()
    )
