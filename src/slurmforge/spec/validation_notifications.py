...
def validate_notifications_contract(spec: ExperimentSpec) -> None:
    ...
    if not email.to:
        raise ConfigContractError(
            "`notifications.email.to` is required when email notifications are enabled"
        )
    if not email.events:
        raise ConfigContractError(
            "`notifications.email.on` must include at least one event"
        )
    ...
    for index, recipient in enumerate(email.to):
        if "@" not in recipient:
            raise ConfigContractError(
                f"`notifications.email.to[{index}]` must be an email address"
            )
    ...
    for event in email.events:
        if event not in options_for("notifications.email.on"):
            raise ConfigContractError(
                f"`notifications.email.on` contains unsupported event: {event}"
            )
