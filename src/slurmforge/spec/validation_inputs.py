...
def validate_stage_inputs_contract(
    spec: ExperimentSpec, stage: StageSpec, *, check_paths: bool
) -> None:
    if stage.depends_on and not any(
        input_spec.required for input_spec in stage.inputs.values()
    ):
        raise ConfigContractError(
            f"`stages.{stage.name}` depends on upstream stages but declares no required inputs"
        )
    for input_name, input_spec in stage.inputs.items():
        ...
        if not inject_mode_matches_expectation(
            input_spec.inject.mode, input_spec.expects
        ):
            raise ConfigContractError(
...
def _validate_upstream_input(
    spec: ExperimentSpec, stage: StageSpec, input_name: str
) -> None:
