...
def parse_notification_events(raw: Any) -> tuple[str, ...]:
    if raw is None:
        return default_for("notifications.email.on")
    if not isinstance(raw, list):
...
def parse_notifications(raw: Any) -> NotificationsSpec:
    ...
    mode = str(email_data.get("mode") or default_for("notifications.email.mode"))
    ...
    return NotificationsSpec(
        email=EmailNotificationSpec(
            ...
            mode=mode,
            from_address=str(
                email_data.get("from") or default_for("notifications.email.from")
            ),
            sendmail=str(
                email_data.get("sendmail")
                or default_for("notifications.email.sendmail")
            ),
            subject_prefix=str(
                email_data.get("subject_prefix")
                or default_for("notifications.email.subject_prefix")
            ),
        )
    )
