...
def _reconciled_attempt(
    *,
    ...
    return StageAttemptRecord(
        ...
        attempt_source="scheduler_reconcile"
        if previous is None
        else previous.attempt_source,
        ...
        if slurm_state.array_task_id is None
        else str(slurm_state.array_task_id),
        ...
        started_at=now
        if previous is None or not previous.started_at
        ...
        if previous is None
        else previous.artifact_manifest_path,
    )


def scheduler_attempt(
    run_dir: Path, *, stage_instance_id: str, slurm_state, terminal: bool
) -> str | None:
    for attempt in list_attempts(run_dir):
        if attempt.scheduler_job_id == slurm_state.job_id or (
