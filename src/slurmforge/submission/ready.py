...
def load_ready_prepared_submission(
    prepared: PreparedSubmission,
) -> tuple[StageBatchPlan, SubmissionLedger]:
    ...
    if materialization is None:
        raise ConfigContractError(
            f"Materialization status is missing for prepared submission `{batch_root}`"
        )
    ...
    if (
        materialization.batch_id != prepared.batch_id
        or materialization.stage_name != prepared.stage_name
    ):
        raise ConfigContractError(
    ...
    if (
        materialization.submit_manifest_path
        and Path(materialization.submit_manifest_path) != prepared.manifest_path
    ):
        raise ConfigContractError(
    ...
    if not prepared.manifest_path.exists():
        raise ConfigContractError(
            f"Prepared submission manifest does not exist: {prepared.manifest_path}"
        )
    ledger = read_submission_ledger(batch_root)
    if ledger is None:
        raise ConfigContractError(
            f"Submission ledger is missing for prepared submission `{batch_root}`"
        )
    ...
    for record in ledger.groups.values():
        if not Path(record.sbatch_path).exists():
            raise ConfigContractError(
                f"Prepared sbatch file does not exist: {record.sbatch_path}"
            )
