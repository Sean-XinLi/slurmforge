...
OPTIONS_BY_PATH = {
    "artifact_store.strategy": ARTIFACT_STRATEGIES,
    "artifact_store.fallback_strategy": ARTIFACT_FALLBACK_STRATEGIES,
    "dispatch.overflow_policy": DISPATCH_POLICIES,
    "notifications.email.mode": EMAIL_MODES,
    "notifications.email.on": EMAIL_EVENTS,
    "runs.type": RUN_TYPES,
    "stages.*.entry.type": ENTRY_TYPES,
    "stages.*.gpu_sizing.estimator": GPU_SIZING_ESTIMATORS,
    "stages.*.inputs.*.expects": INPUT_EXPECTS,
    "stages.*.inputs.*.inject.mode": INPUT_INJECT_MODES,
    "stages.*.inputs.*.source.kind": INPUT_SOURCE_KINDS,
    "stages.*.launcher.mode": LAUNCHER_MODES,
    "stages.*.launcher.type": LAUNCHER_TYPES,
    "stages.*.outputs.*.discover.select": OUTPUT_SELECTORS,
    "stages.*.outputs.*.kind": OUTPUT_KINDS,
}
