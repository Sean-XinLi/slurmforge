...
from .models import GpuHardwareProfile, GpuResourceSizingRequest, GpuSizingDefaults, GpuSizingResolution
    ...
    *,
    ...
    if not _is_auto(raw_gpus_per_node):
        if gpu_type and gpu_type not in gpu_types:
            raise ConfigContractError(f"`stages.{stage_name}.resources.gpu_type` references unknown gpu type `{gpu_type}`")
        if gpu_sizing is not None:
            raise ConfigContractError(f"`stages.{stage_name}.gpu_sizing` is only allowed when gpus_per_node is auto")
    ...
    if not gpu_type:
        raise ConfigContractError(f"`stages.{stage_name}.resources.gpu_type` is required when gpus_per_node is auto")
    if gpu_type not in gpu_types:
        raise ConfigContractError(f"`stages.{stage_name}.resources.gpu_type` references unknown gpu type `{gpu_type}`")
    if gpu_sizing is None:
        raise ConfigContractError(f"`stages.{stage_name}.gpu_sizing` is required when gpus_per_node is auto")

    estimator = gpu_sizing.estimator
    if estimator != "heuristic":
        raise ConfigContractError(f"`stages.{stage_name}.gpu_sizing.estimator` must be heuristic")
    target_memory_gb = gpu_sizing.target_memory_gb
    if target_memory_gb <= 0:
        raise ConfigContractError(f"`stages.{stage_name}.gpu_sizing.target_memory_gb` must be > 0")
    ...
    if memory_gb <= 0:
        raise ConfigContractError(f"`hardware.gpu_types.{gpu_type}.memory_gb` must be > 0")
    if usable_memory_fraction <= 0 or usable_memory_fraction > 1:
        raise ConfigContractError(f"`hardware.gpu_types.{gpu_type}.usable_memory_fraction` must be in (0, 1]")
    ...
    if safety_factor < 1:
        raise ConfigContractError(f"`stages.{stage_name}.gpu_sizing.safety_factor` must be >= 1")
    round_to = gpu_sizing.round_to
    if round_to is None:
        round_to = defaults.round_to
    round_to = int(round_to)
    if round_to < 1:
        raise ConfigContractError(f"`stages.{stage_name}.gpu_sizing.round_to` must be >= 1")
    min_gpus = gpu_sizing.min_gpus_per_job
    max_gpus = gpu_sizing.max_gpus_per_job
    if min_gpus < 1:
        raise ConfigContractError(f"`stages.{stage_name}.gpu_sizing.min_gpus_per_job` must be >= 1")
    ...
    if max_gpus_per_node not in (None, "") and resolved_gpus_per_node > int(max_gpus_per_node):
        raise ConfigContractError(
